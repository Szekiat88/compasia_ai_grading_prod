...
SQS_QUEUE_URL = 'https://sqs.ap-southeast-1.amazonaws.com/046498959242/ai-grading-s3-uat'
...
base_url = "https://d32b6s7249eci7.cloudfront.net/"
...
def create_session():
    ...
    # 3. Insert into ai_session table (api_key_id and imei)
    conn = get_db_connection()
    if not conn:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Update query - ai-session - status: Open
            insert_query = """
                INSERT INTO ai_session (api_key_id, imei, created_at, updated_at)
                VALUES (%s, %s, NOW(), NOW())
                RETURNING session_id
            """
            cur.execute(insert_query, (api_key_id, imei))
                    ...
                    for record in records:
                        ...
                        if result:
                            ...
                            if image:
                                ...
                                # Perform inference
                                try:
                                    results = model.predict(resized_image)  # Perform inference
...
def start_evaluation():
    ...
    #     cur.execute(query, (session_id,))
    #     rows = cur.fetchall()
    #     # Update query - ai-session - status: processing
    #     update_query = """
    #         UPDATE ai_session
    #         SET updated_at = %s,
    #             status = %s
    ...
    try:
        with conn.cursor() as cur:
            ...
            if len(rows) != 6:
                ...
                return jsonify({"error": "Missing or invalid image sides for the given session_id. Please upload all the images again."}), 404
